var j = 1;
function add() {
    var h1 = String (prompt("Please Enter the Heading"));
    var i1 = Number(prompt("Enter Number of Tasks (Enter Only Number)"));
    var array1 =  new Array();
    for (var i = 0; i < i1; i++) {
        array1[i] = prompt("ENTER " + (i+1) + " ELEMENT");
    }
    var boxd = document.getElementById("boxdiv");
    var box1 = document.createElement("div");
    box1.className = "box auto";
    box1.innerHTML += `<h3>${h1}</h3>`;
    for (var i = 0; i < i1; i++) {
        if(array1[i]!=""){
            box1.innerHTML += `<h4>             
                                <input type="checkbox" id="id${j}" onclick="checkl()">
                                <label for="id${j}" id="label${j}">${array1[i]}</label>
                        </h4>`;
            j++;
        }
    }
